...
def get_flist(base_dir):
    ...
    # [csv名称,路径\\csv名称]
    f_list = [[f, os.path.join('countryHistory', f)] for f in
              os.listdir(os.path.join(base_dir, 'countryHistory')) if f.endswith('.csv')]
    ...
    for i, f_info in enumerate(f_list):
        ...
        if 'America' in prov_name:
            prov_name = 'United States'
        elif 'The United Kingdom' in prov_name:
            prov_name = 'United Kingdom'
        elif 'Russian' in prov_name:
            prov_name = 'Russia'
        elif 'Korea' in prov_name:
            prov_name = 'Korea'
        elif 'Iran' in prov_name:
            prov_name = 'Iran'
        elif 'Uzbekstan' in prov_name:
            prov_name = 'Uzbekistan'
        elif 'Yemen' in prov_name:
            prov_name = 'Yemen'
        elif 'Democratic Republic of the Congo' in prov_name:
            prov_name = 'Dem. Rep. Congo'
        elif 'Zambia' in prov_name:
            prov_name = 'Zambia'
        elif 'Bolivia' in prov_name:
            prov_name = 'Bolivia'
...
def get_data(base_dir, f_list):
    # 由于中国的时间最早，故以中国的时间轴作为全时间轴
    hubei_f = [f for f in f_list if f[0] == "China"][0]
    ...
    for i_time, data_timei in enumerate(data):
        ...
        for i_prov, prov_data in enumerate(data_timei['data']):
            percent = prov_data['value'][0] / sum_data * 100  # 某天该国家确诊人数占全世界的百分比，用来画饼状图
...
if __name__ == '__main__':
    ...
    timeline.render("world_current.html")